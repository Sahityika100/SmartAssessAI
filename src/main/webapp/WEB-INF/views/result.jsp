...
<body>
	...
	<nav class="sidebar">
		...
		<div style="flex-grow: 1;">
			<a href="/smartai/sdb" class="nav-link"><i
				...
				href="/test/results" class="nav-link active"><i
    ...
    <header class="header-flex">
        <div>
            <h2 style="font-weight: 800; font-size: 2rem;">Test Results</h2>
            <p style="color: var(--text-body);">Your performance overview</p>
        </div>
    ...
    <c:forEach var="t" items="${tests}">
        <c:set var="totalScore" value="${totalScore + t.score}" />
        <c:set var="totalQuestions" value="${totalQuestions + t.totalQuestions}" />
    </c:forEach>
    ...
    <div class="grid-container">
        ...
        <div class="glass-card" style="max-width:400px; margin:auto;">
            ...
            <p style="text-align:center; margin-top:10px; color:var(--text-body);">
        ...
        <tbody>
            <c:forEach var="test" items="${tests}">
                <tr style="border-bottom:1px solid #e5e7eb;">
                    ...
                    <td style="padding:12px;">${test.course.courseName}</td>
                    <td>${test.totalQuestions}</td>
                    <td>${test.score}</td>

                    <td>
                        <c:set var="percentage" value="${(test.score * 100) / test.totalQuestions}" />
                        ${percentage}%
                    </td>
                    ...
                    <td>
                        <span style="
                            ...
                            border-radius:10px;
                            font-size:0.8rem;
                            background:${test.status == 'Done' ? '#dcfce7' : '#fee2e2'};
                            color:${test.status == 'Done' ? '#16a34a' : '#dc2626'};
                        ">
                             ${test.status == 'Done' ? test.status : 'Undone'}
                        </span>
                    </td>
...
<script>
    ...
    const remaining = totalQuestions - totalScore;
